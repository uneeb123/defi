...
export default class ConnectModal extends React.PureComponent {
  render() {
    const {
      modal, cancel, connecting, connect, connected, disconnect
    } = this.props;
    return (
      <Modal visible={modal} footer={false} onCancel={() => { cancel(); }}>
        {
          connected ?
          (
          ...
          !connecting
            ? (
              <div className="modal-container">
                ...
                <p>To start using Finpound</p>
                <div className="connect-container">
                  <div role="button" tabIndex={-1} className="connect-item" onClick={() => { connect('metamask'); }} onKeyDown={() => { connect(); }}>
                    <img src={Metamask} alt="metamask-logo" />
                    <h1>Metamask</h1>
                    <img src={Arrow} alt="right-arrow" />
                  </div>
                  <div role="button" tabIndex={-2} className="connect-item" onClick={() => { connect('walletconnect'); }} onKeyDown={() => { connect(); }}>
                    <img src={WalletConnect} alt="metamask-logo" />
                    <h1>Wallet Connect</h1>
                    <img src={Arrow} alt="right-arrow" />
                  </div>
                  <div role="button" tabIndex={-3} className="connect-item" onClick={() => { connect('coinbase'); }} onKeyDown={() => { connect(); }}>
